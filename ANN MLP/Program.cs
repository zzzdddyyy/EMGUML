...
namespace ANN_MLP
{
    internal class Program
    {
        ...
        private static void ANNBaes()
        {
            int trainSampleCount = 300;
            ...
            trainData1.SetRandNormal(new MCvScalar(190), new MCvScalar(40));
            Matrix<float> trainData2 = trainData.GetRows(trainSampleCount >> 1, trainSampleCount, 1);
            trainData2.SetRandNormal(new MCvScalar(310), new MCvScalar(40));
            ...
            using (Emgu.CV.ML.ANN_MLP network = new Emgu.CV.ML.ANN_MLP())
            {
                network.SetLayerSizes(layerSizeMat);//网络结构
                network.SetActivationFunction(Emgu.CV.ML.ANN_MLP.AnnMlpActivationFunction.SigmoidSym, 0, 0);//激活函数
                network.TermCriteria = new MCvTermCriteria(10, 1.0e-8);
                network.SetTrainMethod(Emgu.CV.ML.ANN_MLP.AnnMlpTrainMethod.Backprop, 0.1, 0.1);//反馈传播
